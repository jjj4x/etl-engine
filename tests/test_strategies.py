...
# noinspection SqlDialectInspection
class TestHWMIncrementStrategy:
    @staticmethod
    def setup():
        with connect(
            host='postgres',
            ...
            with conn.cursor() as cur:
                cur.execute("drop table if exists test_hwm_increment_strategy")

                cur.execute("""
                    create table test_hwm_increment_strategy (
                        id int primary key,
                        name varchar(255)
                    )
                """)
                ...
                extras.execute_values(
                    cur,
                    "insert into test_hwm_increment_strategy(id, name) values %s",
                    [(i, f'Number: {i}') for i in range(1, 10000 + 1)],
                    page_size=1000,
                )
    ...
    def test_all(self):
        conf = config.ETLConf(
            ...
            etl={
                'source': {
                    'schema': 'public',
                    'name': 'test_hwm_increment_strategy',
                    'hwm': {
                        'expression': 'id',
                        'hwm_type': 'int',
                    },
                ...
                'target': {
                    'location': 'hdfs://hpdc:8020/test_hwm_increment_strategy.parquet',
                },
                'strategy': {
                    'name': 'hwm_increment',
                    'increment': 1000,
        ...
        with sql.SparkSession.builder.config(conf=spark_conf).getOrCreate() as spark:
            reader = io_adapters.JDBCReader()
            writer = io_adapters.HDFSWriter()
            strategy = strategies.HWMIncrementStrategy(reader, writer, metastore)
            strategy.load(spark, conf)

            df = spark.read.load(path=conf.etl.target.location).cache()
            assert df.count() == 1000
            assert df.agg({'id': 'max'}).take(1)[0][0] == 1000
            assert df.agg({'id': 'min'}).take(1)[0][0] == 1
            ...
            assert df.agg({'id': 'max'}).take(1)[0][0] == 2000
